...
(29 разделов приложения), гибрид-поиск + буст по ОКПД2 + LLM-реранкер, навигатор, `POST /navigate`,
Streamlit (форма), петля кейсов (`verified_cases`); ретрив измерен (recall@1 0.98, MRR 0.988),
ответ измерен (`eval_answers.py`: faithfulness 0.95 + рантайм-постпроверка чисел).

...
#
# ⚠⚠ ОДНОБУКВЕННЫЕ ЕДИНИЦЫ — ТОЛЬКО С ЗАГЛАВНОЙ (ревью PR #133). Русские «в» и «а» — предлог и
# союз, и при `re.I` «12 В КОМПЛЕКТЕ» читалось как 12 вольт, «изделия 3 А ТАКЖЕ прочие» — как
# 3 ампера. Выдуманный номинал не может ДОБАВИТЬ порог: он умеет только создать конфликт и
# СНЯТЬ порог — то есть каждая ошибка разбора здесь бьёт в класс «Мочеприемников», который
# проект считает дороже исправляемого. В корпусе единицы пишутся заглавной: «1000 В», «6300 А»,
# «110 кВ». Единица «т» убрана вовсе — слишком двусмысленна и ни разу не нужна.
# ⚠ ДРОБНЫЕ ЗНАЧЕНИЯ ПРОПУСКАЮТСЯ, а не обрезаются: «массой свыше 0,25 кг» прежде давало 25 кг —
# ошибка в СТО РАЗ, и снова в сторону снятия порога.
# ⚠ ПЕРЕЧИСЛЕНИЯ БЕРУТСЯ ЦЕЛИКОМ: «на напряжение 110, 220, 330 и 550 кВ» — это ЧЕТЫРЕ номинала,
# а прежняя форма `[\d\s]*` склеивала «20 30 40 мм» в одно число 203040.
_UNIT_ANY = r"кВт|кВ|мм|кг"          # многобуквенные — регистр не важен
_UNIT_ONE = r"А|В"                   # однобуквенные — ТОЛЬКО заглавная
# ⚠ ЧИСЛА ПЕРЕД ЕДИНИЦЕЙ БЕРУТСЯ ВСЕ (ревью PR #133): «на напряжение 110, 220, 330 и 550 кВ» —
# это ЧЕТЫРЕ номинала. Прежняя форма брала только последнее, и позиция «на напряжение 110 кВ»
# конфликтовала с собственным перечислением — то есть теряла порог.
_RATING_RE = re.compile(
    r"((?:\d+(?:[.,]\d+)?[\s,и]+)*\d+(?:[.,]\d+)?)\s*(?:(?P<any>" + _UNIT_ANY + r")|(?P<one>" + _UNIT_ONE + r"))\b")

# Открытая граница: номинал задаёт ДИАПАЗОН, а не значение. «Измерительные трансформаторы
# напряжением 6 кВ И ВЫШЕ» покрывает и 10 кВ, и 110 кВ — конфликта с ними нет.
_OPEN_BOUND = re.compile(r"и\s+выше|свыше|не\s+менее|более|от\s+\d", re.I)


def _ratings(text: str | None) -> dict[str, set[int]]:
    """Номиналы наименования по ЕДИНИЦАМ: {"а": {4000, 6300}, "в": {1000}}.

    Дробные значения пропускаются: они не служат идентичностью в этом корпусе, а обрезка давала
    ошибку на порядки. Перечисления собираются целиком.
    """
    out: dict[str, set[int]] = {}
    for m in _RATING_RE.finditer(text or ""):
        # ⚠ ДРОБНОЕ ЗНАЧЕНИЕ ОТМЕНЯЕТ ВЕСЬ МАТЧ, а не одно число: после того как перечисления
        # стали разбираться, запятая служит и разделителем списка, и десятичной точкой, и
        # «свыше 0,25 кг» распадалось на {0, 25} — то есть выдумывало номинал 25 кг. Здесь
        # любое сомнение решается в пользу СОХРАНЕНИЯ порога: нет номинала — нет вето.
        if re.search(r"\d[.,]\d", m.group(1)):
            continue
        unit = (m.group("any") or m.group("one")).lower()
        for num in re.split(r"[\s,и]+", m.group(1)):
            if num:
...
def _rating_conflict(product_name: str | None, names: list[str]) -> bool:
    """Номиналы ОДНОЙ единицы названы у обоих, ОГРАНИЧЕНЫ и НЕ пересекаются → строка не про эту
    позицию.
    ...
    примечаниях ни разу. Самый дорогой класс дефекта проекта — правильное число, привязанное не
    к той позиции.

    ⚠⚠ ОТКРЫТАЯ ГРАНИЦА СНИМАЕТ ВЕТО (ревью PR #133). Четыре из девяти строк `strict_name`
    называют себя диапазоном: «напряжением 6 кВ И ВЫШЕ», «35 кВ и выше», «110 кВ и выше».
    Сравнение множеств объявляло конфликт с любым конкретным значением, и позиция «Выключатели
    силовые высоковольтные напряжением 110 кВ» теряла свой порог. Сегодня радиус этого нулевой
    только потому, что имена корпуса совпадают с именами примечаний ДОСЛОВНО, — а следующая
    редакция приложения роняла бы порог молча.
    ...
    ⚠ ВЕТО УМЕЕТ ТОЛЬКО СНИМАТЬ ПОРОГ. Поэтому все сомнения решаются В ПОЛЬЗУ СОХРАНЕНИЯ: любая
    неуверенность (дробное значение, открытая граница, строчная буква единицы) означает «не вето».
    """
    if _OPEN_BOUND.search(_strip_fn(product_name) or ""):
        return False
    ...
    for nm in names:
        clean = _strip_fn(nm)
        if _OPEN_BOUND.search(clean or ""):
            return False        # имя задаёт диапазон — конкретное значение ему не противоречит
        have = _ratings(clean)

...
#
# ЧТО ЧИНИТСЯ. Позиция «Выключатель автоматический низковольтный в литом корпусе (до 1000 В) на
# токи до 2000 А» получала график прим. 27, которое её НЕ НАЗЫВАЕТ: вводная называет ОДНУ позицию
# — «Выключатель автоматический (ВОЗДУШНЫЙ) … на токи до 6300 А». Правило `_name_overlap >= 2`
# пропускало, потому что два наименования делят девять значимых слов.
#
# ⚠⚠ ПОЧЕМУ НЕ ПО НОМИНАЛУ, ХОТЯ ИМЕННО ОН БРОСАЕТСЯ В ГЛАЗА. Первая редакция сравнивала
# номиналы («до 2000 А» против «до 6300 А»), и два раунда ревью показали, что этот путь не
# сходится: «6 кВ И ВЫШЕ» читалось точным значением и роняло порог у «110 кВ»; «110 - 500 кВ»
# теряло нижнюю границу; «массой свыше 0,25 кг» давало 25 кг; предлог «в» и союз «а» становились
# вольтами и амперами. Каждая ошибка разбора — снятый порог, класс «Мочеприемников».
# ⚠ И главное: настоящий различитель этих двух позиций — слово «(ВОЗДУШНЫЙ)», а не ампераж.
# Номинал давал верный ответ по неверной причине.
#
# ⚠ РАДИУС ОБОИХ ПРАВИЛ ИЗМЕРЕН И СОВПАЛ: по 1141 записи ровно ОДНА позиция теряет порог,
# 0 появившихся, 0 изменённых. При равном результате выбрано правило БЕЗ разбора чисел —
# у него нет ни открытых границ, ни диапазонов, ни дробных, ни однобуквенных единиц.
#
# ⚠ Правило локальное: только `strict_name` (9 строк из 419). Обычные групповые строки
# (прим. 7, 37) работают как прежде — там имя законно называет представителя группы, и
# требование дословности снесло бы 18 верных порогов.


# Открытая граница в наименовании: «6 кВ И ВЫШЕ», «свыше 100 кг», «от 4000 А». Такое имя
# описывает КЛАСС изделий, а не одно изделие.
_OPEN_BOUND = re.compile(r"и\s+выше|свыше|не\s+менее|более|от\s+\d", re.I)


def _words(name: str | None) -> frozenset[str]:
    """Значимые слова наименования — без сноски, без регистра, БЕЗ ПОРЯДКА.

    ⚠ Сравнение множеств, а не строк: приложение и примечание пишут одно изделие разным порядком
    слов («Вакуумные контакторы низковольтные» против «Контакторы вакуумные низковольтные»).
    Дословное сравнение роняло бы порог на такой перестановке, а различить «до 2000 А» от
    «до 6300 А» множество слов по-прежнему позволяет: у них разные числа и разный квалификатор.

    ⚠⚠ ЗАПИСАННЫЙ ПРЕДЕЛ: токены короче четырёх знаков выпадают, поэтому позиции, различимые
    ТОЛЬКО коротким числом или буквенным индексом, для этой функции неразличимы. В корпусе таких
    пар девять — «Арматура кабельная … на напряжение 110 - 500 кВ» против «… 6 - 35 кВ»,
    «Хладон 14» против «Хладон-125 ХП», «Жидкость Б-1» против «М-1» и т. п.
    ⚠ Сегодня это безопасно и ПРОВЕРЕНО: ни одна из девяти пар не попадает под строку
    `strict_name` (их коды — 23.19.25, 23.43.10, 27.11.4, 27.12, 27.12.10.110/120/130, 27.12.23,
    27.33), а вне `strict_name` функция не вызывается вовсе. Появится примечание такой формы над
    такой парой — правило привяжет чужой порог, и это надо будет ловить здесь.
    Проверку держит `tests/test_k2_2_strict_name.py::test_short_token_collisions_stay_outside`.
    """
    return frozenset(re.findall(r"\w{4,}", _norm(_strip_fn(name))))


def _names_this_position(product_name: str | None, names: list[str]) -> bool:
    """Строка `strict_name` относится к этой позиции?

    ⚠⚠ ДВА СЛУЧАЯ, И ОНИ РАЗЛИЧАЮТСЯ СТРУКТУРНО (ревью PR #133, раунд 2).
    * Имя называет ОДНО изделие («Выключатель автоматический (ВОЗДУШНЫЙ) … на токи до 6300 А») —
      тогда имя и есть ключ, и совпадение обязано быть ДОСЛОВНЫМ. Иначе порог прим. 27 уходит
      соседней позиции «… на токи до 2000 А», которую примечание не называет: она делит с ним
      девять значимых слов, и правила «≥2 общих слова» не хватает.
    * Имя описывает КЛАСС через открытую границу («Выключатели силовые высоковольтные
      напряжением 6 кВ И ВЫШЕ») — тогда дословность неверна: позиция «… напряжением 110 кВ»
      законно попадает под это описание, 110 ≥ 6. Здесь работает прежнее пересечение слов.

    ⚠ ПОЧЕМУ НЕ ПО НОМИНАЛУ. Первая редакция сравнивала числа, и два раунда ревью показали, что
    путь не сходится: «6 кВ и выше» читалось точным значением, «110 - 500 кВ» теряло нижнюю
    границу, «свыше 0,25 кг» давало 25 кг, предлог «в» и союз «а» становились вольтами и
    амперами. Каждая ошибка разбора — СНЯТЫЙ порог, класс «Мочеприемников». И главное: настоящий
    различитель двух позиций 27.12 — слово «(ВОЗДУШНЫЙ)», а не ампераж; номинал давал верный
    ответ по неверной причине.
    """
    want = _words(product_name)
    if not want:
        return False
    for nm in names:
        clean = _strip_fn(nm)
        if _words(nm) == want:
            return True                      # ТЕ ЖЕ значимые слова — это она
        if _OPEN_BOUND.search(clean or "") and _name_overlap(product_name, [nm]) >= 2:
            return True                      # имя описывает класс — хватает пересечения
    return False
...
def _lookup(codes: list[str], product_name: str, section: str | None, scope: str) -> str | None:
    ...
    if codes:
        ...
        for r in _flat_thresholds():
            ...
            # ⚠⚠ У `strict_name` ИМЯ — КЛЮЧ, значит совпадение ДОСЛОВНОЕ (`K2-2` #128).
            # Пересечения «≥2 значимых слова» здесь мало: наименования, различающиеся только
            # квалификатором «(воздушный)» и номиналом, делят девять слов, и порог прим. 27
            # уходил позиции, которую это примечание не называет. Обоснование выбора именно
            # такого правила — в шапке `_names_this_position`.
            if r.get("strict_name") and not _names_this_position(product_name, r["names"]):
                continue

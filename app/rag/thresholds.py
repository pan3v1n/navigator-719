...
# ⚠ РАЗДЕЛЯЕМ ТОЛЬКО ПО ЯВНОМУ «…КОТОРЫЕ ОЦЕНИВАЮТСЯ» (ревью 20.08.2026, зеркало исходного
# дефекта). Признака «после „за исключением“ где-то есть „не менее“» НЕ ХВАТАЕТ: оговорка бывает
# и обычным сужением охвата, за которым продолжается ОБЩИЙ график —
#   «до 30 июня 2023 г. не менее 250 баллов, за исключением судов рыбопромыслового флота,
#    с 1 июля 2025 г. не менее 300 баллов»
# Здесь «не менее 300» принадлежит общему графику, и слабый признак утащил бы действующую ступень
# в строку «ИНОЙ порог», оставив в «Порог» ПРОСРОЧЕННЫЕ 250 — ровно та ошибка, ради которой
# разделение и вводилось, только в другую сторону. В прим. 17 собственный график оговорки всегда
# вводится глаголом оценки («и которые оцениваются до 30 июня 2025 г. не менее 2300 баллов»).
# Не опознали — НЕ делим: строка останется прежней, а не станет неверно подписанной.
_EXC_SCHEDULE_RE = re.compile(r"оценива", re.IGNORECASE)
...
def _split_exception(text: str) -> tuple[str, str | None]:
    ...
    if not m:
        return text, None
    tail = m.group(1)
    if "не менее" not in tail or not _EXC_SCHEDULE_RE.search(tail):
        return text, None
    general = text[:m.start()].rstrip(" ,;")
    if not general:                      # оговорка съела бы весь порог — молчим, а не печатаем пусто
        return text, None

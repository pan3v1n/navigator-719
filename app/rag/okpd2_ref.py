...
_TNVED_AMOUNT_BEFORE_RE = re.compile(
    r"(?:сумм\w*|оборот\w*|стоимост\w*|цен[аыу]|выручк\w*|бюджет\w*|контракт\w*)"
...
def extract_tnved_position(text: str | None) -> str | None:
    ...
    for m in _TNVED_POSITION_RE.finditer(t):
        ...
        if _TNVED_NOT_A_CODE_RE.match(t[m.end():m.end() + 16]):
            continue  # год, сумма, количество — слово ПОСЛЕ числа называет это прямо
        if _TNVED_AMOUNT_BEFORE_RE.search(t[:m.start()]):
            continue  # «сумма контракта 250000» — слово ПЕРЕД числом

...
def format_context(hits: list[Hit], query: str | None = None) -> str:
    # Требования показываем ТОЛЬКО у целевого хита (совпадение по коду ОКПД2, иначе top-1); прочие
    # кандидаты идут строкой «наименование + код» как материал для уточнения — см. EV7 ниже.
    has_match = any(h.okpd2_match for h in hits)
    # ⚠ Кто целевой — решает ОДНО место (`_target_hit`), иначе правка «целевой» починила бы метрику
    # и таблицу баллов, но не сам контекст: у `format_context` было своё независимое `i == 1`, и на
    # светодиодах полный кап операций (60) достался бы строке-квалификатору с обрывком требований,
    # а содержательная позиция осталась бы усечённой до 12 из 15 (EV6). Ветка совпадения по коду
    # сохранена дословно: там целевыми остаются ВСЕ совпавшие записи — у кода 26.11.22.210 их две,
    # и требования поделены между ними, так что сужать до первой нельзя.
    preferred = _target_hit(hits) if not has_match else None
    blocks: list[str] = []
    for i, h in enumerate(hits, 1):
        is_target = h.okpd2_match if has_match else (h is preferred)
        ...
        # кандидатах работало, а на боевом окне 8 держать перестало: гейт «без чужих чисел» стоял на
        # 0.90, и утечка шла с одного запроса — «центробежные насосы», 3 прогона из 5, числа
        # 110/450/750/850 позиции «Насосы технологические типов ВВ1… для крупнотоннажных производств
        # СПГ». Числа дословно верны и заземлены в контексте, поэтому ни `unverified_numbers`, ни
        # глаз эксперта подмены не видят: читается как «порог для моих насосов 450 баллов».
        #
        ...
        #
        # ЦЕНА проверена по трафику, а не предположена: из 580 уникальных вопросов июльской волны
        # просьба сравнить позиции ровно ОДНА («можешь сравнить требования по нашему коду и по
        # 26.30.50») — и она С КОДОМ, то есть обе записи совпадают по коду и обе остаются целевыми.
        # Кандидаты и по докстрингу `_plan_answer` служат материалом для УТОЧНЕНИЯ по коду
        # (правило 1г), а для этого нужны наименование и код, а не баллы.
        #
        ...
        if not is_target:
            n_ops = _n_operations(h)
            if not n_ops:  # своих требований нет — считаем требования ГРУППЫ (R6), иначе «0» читается как «их нет»
                parent = inheritance.lookup(h.section_roman, h.product_name)
                n_ops = len(parent.get("operations") or []) if parent else 0
            count = f" (в базе {n_ops} операц.)" if n_ops else ""
            lines.append(
                f"    Требования этой позиции в контекст НЕ включены{count} — она НЕ целевая. Это "
                "кандидат для УТОЧНЕНИЯ: если продукция заявителя — эта позиция, попроси её код "
                "ОКПД2 и разбери отдельным ответом. НЕ утверждай, что требований у неё нет, и НЕ "
                "приводи по ней баллов, порогов и процентов — их здесь нет и брать их негде."
            )
            if h.source_anchor:
...
def _target_hit(hits: list[Hit]) -> Hit | None:
    ...
    обрывочной записи, тогда как содержательная лежит в том же окне; метрика «чужие числа» видела
    это как утечку 5 прогонов из 5, и была права по форме.
    Пометка неполноты (`fragments.NOTICE`) остаётся у всей группы — требования там действительно
    расколоты, и состав подтверждает эксперт (`R29-4`). Здесь исправлен только выбор ОПОРЫ.

    Совпадение по коду правило НЕ отменяет: назвал пользователь 26.11.22.210 — опираемся на неё,
    код авторитетнее любого ранга."""
    if not hits:
        return None
    matched = next((h for h in hits if h.okpd2_match), None)

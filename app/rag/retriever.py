...
_RULES_TOPIC = (
    # состав документов, акт экспертизы, СТ-1, экспертиза — порядок ТПП
    ("tpp_order_52", re.compile(
        r"как(ие|ой)?\s+документ|состав\s+документ|перечень\s+документ|пакет\s+документ|"
        r"акт\s+экспертиз|экспертиз|ст-?1|сертификат\s+о\s+происхожд|заключени|"
        r"что\s+(нужно|надо)\s+(собрать|подготовить|предостав)", re.I)),
    # процедура реестра: подача, сроки, ГИСП, реестровая запись, изменения
    ("rules_registry", re.compile(
        r"срок|подач|подать|заявлени|реестров(ая|ой)\s+запис|внесени\w*\s+изменени|"
        r"гисп|каталог|рассмотрен|отказ\w*\s+во?\s+включени|выписк|как\s+внести|"
        r"порядок\s+(включени|внесени|формирован)", re.I)),
    # критерии подтверждения производства — тело постановления
    ("decree_body", re.compile(
        r"критери|подтверждени\w*\s+производств|пункт\s*1|подпункт|"
        r"российск\w*\s+промышленн\w*\s+продукц", re.I)),
)
...
def rules_topic(query: str) -> str | None:
    """`doc_type` документа, которому адресован процедурный вопрос, либо None."""
    for doc_type, pat in _RULES_TOPIC:
...
def search_rules(query: str, limit: int = 6, qvec: list[float] | None = None) -> list[dict]:
    ...
    out: list[dict] = []
    for i in sorted(chosen)[:limit]:  # порядок — исходный ранг гибрида

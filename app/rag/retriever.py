...
_RULES_TOPIC: tuple[tuple[str, tuple[re.Pattern, ...]], ...] = (
    ...
    ("tpp_order_52", (
        ...
        re.compile(r"(?:акт\w*\s+)?экспертиз"
                   r"(?!\w*\s+(?:по\s+определению\s+)?(?:стран\w*\s+)?происхожден)"
    ...
    ("polozhenie49_tpp", (
        re.compile(r"экспертиз\w*\s+(?:по\s+определению\s+)?(?:стран\w*\s+)?происхожден", re.I),
        re.compile(r"(?:акт\w*|провед\w*|срок\w*|порядок)\s+экспертиз\w*\s+"
                   r"(?:по\s+определению\s+)?(?:стран\w*\s+)?происхожден", re.I),
        re.compile(r"документ\w*[^.;|]{0,40}(?:подтвержда|представля)\w*"
                   r"[^.;|]{0,30}(?:критери|происхожден)", re.I),
        ...
        re.compile(r"(?:документ|сведени)\w*[^.;|]{0,40}(?:для|при)\s+(?:проведени\w*\s+)?"
                   r"экспертиз\w*[^.;|]{0,20}происхожден", re.I),
    )),

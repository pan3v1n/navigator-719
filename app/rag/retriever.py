...
_ASKS_DOC_LIST = (
    # ⚠ Разрыв до двух слов обязателен: «какие ИМЕННО документы из подпункта „а“…» — реальный
    # вопрос волны, и он встречается трижды. Без разрыва он не ловился вовсе, а тема уходила в
    # «критерии» (в вопросе назван подпункт), то есть ответ получал не те оговорки.
    re.compile(r"как\w*(?:\s+\w+){0,2}\s+документ|перечен\w*\s+документ|список\s+документ|"
    ...
    re.compile(r"как\w*\s+документ\w*\s+подтверд|чем\s+подтвердить", re.I),
)

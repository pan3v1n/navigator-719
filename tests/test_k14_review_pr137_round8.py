...
class TestCounterWordDisqualifiesABareNumber(unittest.TestCase):
    """⚠⚠⚠ HIGH-5 РАУНДА 8 ОТМЕНЕНА РАУНДОМ 9, И ЭТО РЕШЕНИЕ, А НЕ ОТКАТ ПО НЕДОСМОТРУ.

    Раунд 8 требовал, чтобы счётчик количества (`шт|штук|позиц|единиц`) НЕ опровергал голую
    группу: «ТН ВЭД 1902 позиций» → 1902, потому что 1902 — настоящая товарная позиция.
    Разделение было сделано, и раунд 9 показал, что оно ВОЗВРАЩАЕТ класс «уверенный неверный
    код», ради которого список заводился:
        «по ТН ВЭД поставлено 1500 единиц…» → 1500 · «отгружено 5000 шт…» → 5000
        «по ТН ВЭД 2026 позиций…» → 2026   ← HIGH второго раунда, год как код
    Все три получают тему `st1_origin`, и лукап печатает в промпт «Код ТН ВЭД 1500: в Перечень НЕ
    включён» под указанием «использовать ТОЛЬКО это значение».

    ⚠⚠ РАЗДЕЛИТЬ ЭТИ ФОРМЫ ЛЕКСИЧЕСКИ НЕЛЬЗЯ: «ТН ВЭД 1902 позиций» и «по ТН ВЭД 2026 позиций» —
    одна и та же форма (маркер, четыре цифры, счётчик), оба числа в диапазоне лет, и оба МОГУТ
    быть настоящими позициями. Значит одно из требований обязано проиграть, и выбор делает
    направление ошибки: неверный код дороже отсутствующего.
    """

    def test_bare_number_before_a_counter_is_not_a_code(self):
        for text in ("по ТН ВЭД поставлено 1500 единиц, какие условия происхождения товара",
                     "отгружено 5000 шт по ТН ВЭД, какие условия происхождения",
                     "по ТН ВЭД 2026 позиций, какие условия достаточной переработки",
                     "ТН ВЭД 1902 позиций"):
            with self.subTest(text=text):
                self.assertIsNone(extract_tnved_position(text))

    def test_price_paid_for_the_decision_is_named(self):
        """⚠ Цена названа вслух: вместе с количествами теряются и эти две формы раунда 8.

        Тест закрепляет ПОТЕРЮ намеренно — чтобы следующий раунд не «чинил» её молча, не измерив
        заново, чем она оплачена. Вернуть их можно только вместе с ответом на вопрос, чем
        «2026 позиций» отличается от «1902 позиций»."""
        self.assertIsNone(extract_tnved_position("какие условия для ТН ВЭД 8403 позиция"))
        self.assertIsNone(extract_tnved_position("код ТН ВЭД 840310 позиций"))

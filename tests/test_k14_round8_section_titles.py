...
class TestReportedFixWasRejectedOnPurpose(unittest.TestCase):
    ...
    def test_section_record_numbering_does_not_collide_with_real_points(self):
        """Уникальность ключа `by_key` в выдаче ОДНОГО парсера — Приказа №14.

        ⚠⚠⚠ ДОКСТРИНГ ПЕРЕПИСАН: он описывал нумерацию, которую заменила сама MED-4, и обещал
        срабатывание, ставшее невозможным (находка 8 десятого раунда ревью). Стояло:
        «`_section_records` нумерует запись раздела как `section_roman.k` … Сейчас коллизии нет,
        потому что вводных записей нет вовсе. Тест упадёт ровно в тот момент, когда кто-то
        заведёт их, не починив нумерацию». Обе половины ложны: нумерация теперь
        `section_roman.§k`, записей раздела в корпусе ТРИНАДЦАТЬ (10 у Соглашения СНГ, 3 у
        Приказа №14), а `§`-номер не может совпасть с пунктом `_K14_POINT_RE` ПО ПОСТРОЕНИЮ —
        то есть на названный сценарий тест не упадёт никогда.

        ⚠ Что он проверяет НА САМОМ ДЕЛЕ: ключ уникален внутри `parse_prikaz14`. Это НЕ сторож
        схемы нумерации (её держит `test_k14_review_round10.TestSectionRecordNumbering…`
        мутацией) и НЕ сторож корпуса: девять ЖИВЫХ коллизий стоят у Приказа №52 и сносок, и
        сюда они не попадают по построению — их держит `TestParentIntroNeverTakesAnAmbiguousParent`.
        Читатель, который поверит этому тесту как общему, решит, что коллизии в корпусе
        отслеживаются; они отслеживаются в другом месте."""
        recs = _recs(self)

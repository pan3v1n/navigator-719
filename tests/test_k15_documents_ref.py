...
class TestGuardSparesTheDenial(unittest.TestCase):
    ...
    def test_denial_needs_a_subject_it_can_deny(self):
        ...
        # ⚠⚠ ФОРМЫ ВОСПРОИЗВОДЯЩИЕ, А НЕ ПРАВДОПОДОБНЫЕ (ревью PR #127, раунд 5). Первая
        # редакция брала «при отсутствии продукции» — а прежняя регулярка искала основу
        # «отсутству», формы «отсутствии» не покрывающую, и тест проходил ОДИНАКОВО на обоих
        # версиях кода. Тот же фиктивный контроль, о котором предупреждает `documents_ref`.
        for phrase in ("продукция отсутствует в приложении",
                       "такие сведения отсутствуют в реестре",
                       "продукция также отсутствует в перечне"):

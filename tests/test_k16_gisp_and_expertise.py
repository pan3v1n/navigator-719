...
class TestTopicQuotaKnowsTheNewDocuments(unittest.TestCase):
    ...
    #: Формы вопроса об экспертизе происхождения — включая ПРЕДЛОЖНЫЕ.
    #  ⚠⚠ Первая редакция знала ровно ЧЕТЫРЕ строки, и на них же настраивался шаблон: «экспертиза
    #  О происхождении», «экспертиза НА происхождение», «для определения страны происхождения»
    #  продолжали уходить Приказу №52 (находка раунда 12). Набор расширен предложными формами.
    ORIGIN_FORMS = (
        ...
        "что входит в акт экспертизы о происхождении товара",
        "экспертиза на происхождение товара",
        "какие документы подтверждают критерий полностью полученного товара",
    )

    def test_origin_expertise_wins_the_topic_outright(self):
        """⚠⚠⚠ ПРОВЕРЯЕТСЯ `rules_topic`, А НЕ ПАРНОЕ СРАВНЕНИЕ С №52 (находка раунда 12).

        Прежняя редакция утверждала `s['polozhenie49_tpp'] > s['tpp_order_52']` и была зелёной
        при счёте 1:0 — хотя `rules_topic` в этот момент возвращал **None**, потому что документ
        делил первое место с `sng_origin_rules`. Ничья снимает `primary` у ОБОИХ, то есть тест
        подтверждал победу ровно там, где её не было ни у кого.
        Предметы разные: у №52 акт экспертизы подтверждает ПРОИЗВОДСТВО в РФ для реестра, у №49
        экспертиза определяет СТРАНУ ПРОИСХОЖДЕНИЯ для СТ-1."""
        from app.rag import retriever as R
        for q in self.ORIGIN_FORMS:
            with self.subTest(q=q):
                self.assertEqual(R.rules_topic(q), "polozhenie49_tpp",
                                 f"тема не досталась Положению №49: счёт {self._scores(q)}")
    ...
    #: Вопросы СОСЕДЕЙ — новая строка не должна ни забирать их, ни превращать в ничью.
    #  ⚠⚠ Третий шаблон в первой редакции требовал «критери» без «происхожден» и срабатывал на
    #  обычных реестровых вопросах: победы тела ПП №719 и Приказа №52 превращались в НИЧЬИ, а
    #  ничья снимает `primary` у обоих (находка раунда 12).
    NEIGHBOUR_FORMS = {
        "какие документы нужны для получения акта экспертизы": "tpp_order_52",
        "что такое акт экспертизы и когда он нужен": "tpp_order_52",
        "какие сведения о производственных площадках": "tpp_order_52",
        "документы, подтверждающие соблюдение критериев для реестра": "decree_body",
        "что такое достаточная переработка": "sng_origin_rules",
        "что такое кумулятивный принцип": "sng_origin_rules",
        "как получить сертификат СТ-1": "prikaz14_tpp",
    }

    def test_neighbours_keep_their_own_subjects(self):
        """⚠⚠⚠ ЦЕНА ПРАВКИ — НА СОСЕДЯХ, И СЧИТАЕТСЯ ОНА `rules_topic`, а не парным счётом."""
        from app.rag import retriever as R
        for q, want in self.NEIGHBOUR_FORMS.items():
            with self.subTest(q=q):
                self.assertEqual(R.rules_topic(q), want,
                                 f"сосед потерял свой предмет: счёт {self._scores(q)}")

    def test_the_origin_tail_has_a_single_definition(self):
        """⚠⚠ Хвост «…происхождения» нужен В ДВУХ местах: у №49 и в запрете у №52.

        Две копии — это «две таблицы про одно» (HIGH-2 восьмого раунда): правка одной молча
        расходится со второй, и документ одновременно выигрывает тему и остаётся запрещённым
        соседу. Проверяется, что обе стороны используют ОДНУ константу."""
        from tests._source_tools import code_only
        src = code_only(ROOT / "app" / "rag" / "retriever.py")
        self.assertGreaterEqual(src.count("_ORIGIN_TAIL"), 3,
                                "хвост перестал быть общим — ищите вторую копию выражения")
        self.assertEqual(src.count("(?:стран\\w*\\s+)?происхожден"), 1,
                         "выражение хвоста встречается больше одного раза — это вторая копия")

...
TARGETS: dict[str, tuple[int | str, str]] = {
    # VII и VIII добавлены 2026-07-05 (актуализация под редакцию 01.07.2026): редакция
    # изменила их ИСХОДНИК (N 1392 — фарма VIII бинарная→балльная; N 351 — протезы VII).
    # parse_rtf нарезал их верно, но при смене исходника чанк надо перегенерировать — режем
    # из pp719_full.txt по СУЩЕСТВУЮЩИМ префиксам 07/08 (перезапись на месте, слаги как у файлов).
    "VII": ("07", "VII_medizdeliya"),
    "VIII": ("08", "VIII_farmacevtika"),

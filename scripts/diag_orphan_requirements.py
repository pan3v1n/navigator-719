...
# R29/EV9: РОЛЬ строки расколотой ячейки — факт ДАННЫХ, а не свойство прозы наименования.
#
# Строка вида «Светодиоды (в части светодиодов белого диапазона)» продукцию не называет: скобка
# задаёт ОБЛАСТЬ действия строки кода, а сам продукт назван соседней строкой той же ячейки. Рантайму
# такую строку нельзя брать опорой ответа (`EV6`).
#
# ⚠ ПОЧЕМУ РОЛЬ ПРОСТАВЛЯЕТСЯ ЗДЕСЬ, А НЕ В РАНТАЙМЕ. Раньше `pipeline` решал это регуляркой по
# наименованию — и под неё подходили 16 записей корпуса, из которых 13 НАСТОЯЩАЯ ПРОДУКЦИЯ
# («Медицинские маски (за исключением полумасок FFP1…)», «Краны грузоподъемные прочие
# (за исключением …)», «Громкоговорители …»). Спасало их лишь то, что они не входят в расколотые
# группы; в день, когда эксперт расширит состав групп (`R29-4`), ответ по такой позиции молча уехал
# бы на сиблинга — класс ошибки, против которого R29 и заведён. Здесь эвристика применяется ТОЛЬКО
# внутри уже отобранных групп, результат виден в JSON и правится экспертом, а рантайм читает поле.
_SCOPE_QUALIFIER_RE = re.compile(r"\((?:в\s+части|за\s+исключением)\b", re.I)


def _row_role(name: str | None) -> str:
    """`qualifier` — строка задаёт область действия кода; `product` — называет продукцию."""
    return "qualifier" if _SCOPE_QUALIFIER_RE.search(name or "") else "product"

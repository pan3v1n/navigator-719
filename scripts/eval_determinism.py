...
def foreign_numbers(query: str, hits: list) -> set[str]:
    ...
    `_target_hit` брал `hits[0]` — строку «Светодиоды (в части светодиодов белого диапазона)», —
    а требования лежат в отдельной записи «Светодиоды белого диапазона», и ответ опирается на неё
    законно. Замер показал, что поправка не работает НИ В ОДНУ сторону:
    ...
    Метрика оставлена строгой: числа любой нецелевой записи считаются чужими. Утечка на светодиодах
    была не шумом метрики, а вторым дефектом — и метрика правильно сделала его видимым.

    ✅ ЧИНИЛСЯ ДЕФЕКТ, А НЕ МЕТРИКА (`EV6`, 17.08.2026). Первый диагноз — «пустая запись-дубль
    обгоняет содержательную» — оказался НЕВЕРЕН: сверка с первоисточником показала, что строка
    26.11.22.210 не пуста и не дубль. Это КВАЛИФИКАТОР — одна ячейка требований первоисточника,
    растёкшаяся при конвертации по трём строкам кода, и на квалификаторе осела её голова
    («сборочные чертежи; технические условия; …», 4 операции), а продукция названа строкой
    26.11.22.216 (4 блока, 15 операций). Обе давно помечены в `fragmented_requirements.json` (R29).
    Исправлено в `_target_hit`: целевой не может быть строка-квалификатор, если в окне есть
    содержательный сиблинг ТОЙ ЖЕ расколотой группы.

    ✅ СТРОГОСТЬ МЕТРИКИ ОКУПИЛАСЬ (`EV7`, 17.08.2026). Оставленная строгой (числа любой нецелевой
    записи — чужие), метрика довела до настоящей причины: баллы и пороги кандидатов вообще не должны
    попадать в контекст. Требования нецелевых кандидатов из него убраны — гейт 0.90 → **1.00 в трёх
    прогонах, ноль утечек из 50**. Смягчи её тогда «по ветке кода» — и чинить было бы нечего.

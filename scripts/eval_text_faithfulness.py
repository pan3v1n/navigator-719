...
20 кейсах»). Утверждение ответа заземлено, если его словесные **2-граммы** встречаются в контексте
(почему именно 2, а не 3 — см. комментарий к `N` ниже: на 3-граммах ручная валидация показала, что
свёрнутый в скобки перечень теряет покрытие целиком, хотя каждый его термин заземлён).
...
3. **Считаем ДОЛЮ покрытых n-грамм, а не факт совпадения**: пересказ своими словами законен,
...
def _ngrams(text: str, n: int = N) -> set[tuple[str, ...]]:
    """N-граммы текста ПЛЮС юниграммы.

    ⚠ Юниграммы обязательны, и без них метрика врала в одну сторону. Короткое утверждение
    («- Теплообменники [1]» — одно слово) давало только 1-кортеж, а контекст — только 2-кортежи,
    так что пересечение было пустым ВСЕГДА: наименование позиции, скопированное дословно,
    объявлялось незаземлённым. Такие ложные срабатывания попадали в те самые «11 слабых
    утверждений», по которым методика и была признана негодной."""
    toks = tokenize(text)
...
def report(rows: list[dict], threshold: float, show_claims: bool) -> list[str]:
    ...
    if n_claims:
        L += [f"  ЗАЗЕМЛЁННЫХ (доля {N}-грамм в контексте ≥ {threshold}): "
              ...
              f"  СО СЛОВАМИ ВНЕ КОНТЕКСТА (второй сигнал, кандидаты в выдумку): {novel_n}/{n_claims}",
...
def main() -> None:
    ...
    ap.add_argument("--threshold", type=float, default=0.5,
                    help=f"доля покрытых {N}-грамм, ниже которой утверждение считается незаземлённым")

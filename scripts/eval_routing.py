...
SETS: tuple[tuple[str, str, str], ...] = (
    ...
    # Отдельным файлом, а не в `eval_golden_negative`: тот кормит ПОРОГОВУЮ метрику отказа,
    # и восемь легко проходящих кейсов подняли бы её 0.87 -> 0.89 без единой правки кода.
    ("eval_offdomain_tnved.json", "cases", "вне сферы"),  # 12
)

...
def summarize(results: list[dict], meta: dict) -> dict:
    ...
    # ⚠⚠ ДЛЯ ДЕФЕКТА ИНСТРУМЕНТА — ДОПУСК В ОДНО СОБЫТИЕ, А НЕ ДОЛЯ (ревью PR #135, раунд 4).
    # Раунд 3 поставил «доля > 1 %», и это было МЁРТВЫМ ПРАВИЛОМ: порог достижим только при
    # n >= 100, а драйвер гоняет 30 (одиночный) и 10 / 25 / 50 (нагрузка) — то есть один побег
    # по-прежнему обнулял ВЕСЬ оплаченный оконный прогон, ровно как до «починки».
    # ⚠⚠ А ТЕСТ ЭТОГО НЕ ПОЙМАЛ, ПОТОМУ ЧТО БЫЛ НАПИСАН НА n=100 — популяции, которой драйвер
    # НЕ ПРОИЗВОДИТ: зелёный тест при неизменившемся поведении.
    # Правило теперь в СОБЫТИЯХ: одно терпим и ПОМЕЧАЕМ, два и больше (либо свыше 1 % на
    # больших прогонах) — замер недействителен. Отличие от 429 прежнее: тот приходит бурей и
    # заражает соседей, побег из `ask` — событие одного запроса.
    tool_errors = by_class.get(TOOL_ERROR, 0)
    tool_budget = max(1, math.ceil(0.01 * len(results))) if results else 0
    invalid = {k: v for k, v in by_class.items()
               if k in INVALIDATING and (k != TOOL_ERROR or tool_errors > tool_budget)}

    out = dict(meta)
    if tool_errors and tool_errors <= tool_budget:
        out["⚠ дефект инструмента"] = (
            f"{tool_errors} побег(ов) из `ask` — в пределах допуска ({tool_budget}); замер "
            "оставлен действительным, но это НЕ отказ сервиса и в долю отказов не входит")
    out["n"] = len(results)

...
# ровно класс `D12` («записи-обрывки»), от которого этот же файл и защищается. Плюс он поднял бы
# латентную MED-4: вводная нумеруется `section_roman.k`, и на ДЕСЯТИ разделах из одиннадцати
# `{part}.{sec}.1` уже занят настоящим пунктом — `by_key` молча оставил бы последнюю запись, и
# подпункт получил бы преамбулу раздела в `parent_intro`.

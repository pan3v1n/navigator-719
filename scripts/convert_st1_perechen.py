...
# ⚠⚠ ПОСЛЕ РАУНДА 5 ЭТО ЕДИНСТВЕННЫЙ ЗАМОК, И КОММЕНТАРИЙ ИСПРАВЛЕН. Раундом 4 правило продолжения
# требовало РОВНО ОДНОЙ значимой ячейки и потому само отсекало строки главы — отсюда прежняя запись
# про эшелонирование. Но то же правило выбрасывало условия настоящих позиций, где вместе с условием
# переносилось имя, и раундом 5 оно возвращено к честному виду («последняя ячейка — условие»).
# Теперь строку главы отсекает ТОЛЬКО этот шаблон: мутация ставится на него одного.
...
def parse(text: str) -> tuple[list[dict], dict]:
    ...
    # ⚠⚠ ТАБЛИЦА КОНЧАЕТСЯ СЛЕДУЮЩИМ ПРИЛОЖЕНИЕМ (находка HIGH-1 пятого раунда ревью PR #137).
    # Без границы в разбор попадали БЛАНКИ сертификата (приложения 2–5): их графы («4. N ______»,
    # «11. Номер и дата счета-фактуры», «Печать») — тоже строки с чертами, и они конкурировали с
    # настоящими продолжениями ячеек. Из-за них правило продолжения пришлось сузить до «ровно одна
    # значимая ячейка», а это, в свою очередь, выбросило условия у ПЯТИ настоящих позиций.
    # Граница снимает причину: за ней разбирать нечего, и правило продолжения можно вернуть к
    # честному виду — «последняя ячейка строки таблицы и есть условие».
    _end = re.search(r"^\s*Приложение\s+2\b", text[start:], re.MULTILINE)
    body = text[start:start + _end.start()] if _end else text[start:]
    ...
    for line in body.splitlines():
        ...
        if not m:
            ...
            # ⚠⚠ БЕРЁМ ПОСЛЕДНЮЮ ЯЧЕЙКУ, А НЕ ТРЕБУЕМ ЕДИНСТВЕННОЙ (находка HIGH-1 пятого раунда).
            # Редакция раунда 4 принимала продолжение только при ОДНОЙ значимой ячейке — и когда
            # вместе с условием переносилась колонка ИМЕНИ, условие выбрасывалось целиком: пять
            # настоящих позиций (8535, 8536, 1504, 880400000, 902129000) остались с ПУСТЫМ
            # условием, а 853990800 — с обрывком собственного имени. Хуже загрязнения, которое
            # чинилось: лукап печатал «включён в Перечень» и не приводил условия вовсе.
            # ⚠ Почему теперь можно: бланки сертификата отрезаны границей приложения выше, и
            # многоячеечные строки внутри таблицы — это ровно перенос имени, где условие идёт
            # ПОСЛЕДНЕЙ графой. Положительный контроль на пустое условие стоит в `controls`.
            cells = [c.strip().replace("\xa0", "") for c in line.split("|")]
            meaningful = [c for c in cells if c]
            # ⚠ ЧЕРТА ОБЯЗАТЕЛЬНА: строка БЕЗ неё — не продолжение ячейки таблицы, а перенос
            # ИМЕНИ («9405 21 00, 9405 31 000,» под позицией 8539 90 800). Требование стояло в
            # исходной редакции, я его снял правкой раунда 4 — и обрывки имени поехали в условие.
            if last_group and meaningful and "|" in line:
                for idx in last_group:
                    rows[idx]["condition"] = (rows[idx]["condition"] + " " + meaningful[-1]).strip()
...
def controls(rows: list[dict], forms: dict) -> list[str]:
    ...
    # ⚠⚠ НИ ОДНОГО ПУСТОГО УСЛОВИЯ, А НЕ «МЕНЬШЕ 10 %» (находка HIGH-1 пятого раунда ревью PR #137).
    # Порог 10 % не увидел, как правка раунда 4 выбросила условия у ПЯТИ настоящих позиций
    # (8535, 8536, 1504, 880400000, 902129000): 5 из 245 — это 2 %, контроль промолчал. А цена
    # одной такой записи максимальна: лукап печатает «включён в Перечень условий» и НЕ приводит
    # условия — модель получает утверждение, что общее правило не применяется, и ничего взамен.
    without_condition = [r for r in rows if not r["excluded"] and not r["condition"].strip()]
    if without_condition:
        problems.append(f"без условия остались записи: {[r['code'] for r in without_condition]} — "
                        "третья графа не доехала")
    # ⚠ Условие — отглагольное существительное («Изготовление», «Операции», «Пиление»…), а не
    # цифры: начало с цифры означает, что в графу условия уехал обрывок ИМЕНИ или кода.
    # Куплено дефектом: у 8539 90 800 условие начиналось «9405 21 00, 9405 31 000, 9405 41 00 или».
    digit_start = [r["code"] for r in rows if re.match(r"^\d", r["condition"] or "")]
    if digit_start:
        problems.append(f"условие начинается с цифры: {digit_start} — в третью графу уехал "
                        "обрывок наименования")
    # ⚠ Разделитель графы внутри значения — признак склейки соседних ячеек или чужих строк.
    polluted = [r["code"] for r in rows if "|" in (r["condition"] or "")]
    if polluted:
        problems.append(f"в условии остался разделитель графы: {polluted} — склеены соседние "
                        "ячейки или чужая строка таблицы")

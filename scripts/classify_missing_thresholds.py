...
порог» — 50.9 % по срезу `points`+`mixed` (53.1 % до разбора четырёх новых форм примечаний).
Но эта доля складывает ДВА разных явления, и потому не даёт починить ни одно:
    ...
    for path in sorted(glob.glob(CORPUS)):
        for rec in _items(path):
            ...
            info.update({
                ...
                # Якорь первоисточника: код и наименование позицию НЕ определяют. В разделе IV
                # «Устройства наведения промышленные» (26.40.33) стоят ДВАЖДЫ — позиции 11 и 34,
                # с разными требованиями. Без якоря такие строки в листе сверки эксперта выглядят
                # дублем, и «проверьте эту позицию» становится неадресным.
                "anchor": rec.get("source_anchor") or "",
            })
            out.append(info)
    return out

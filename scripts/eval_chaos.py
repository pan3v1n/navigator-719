...
def judge(before: dict, during: dict, after: dict, recovery_seconds: float | None = None,
    ...
    # ⚠⚠ «НЕ ИЗМЕРЕНО» ТОЛЬКО ЕСЛИ НАБЛЮДЕНИЯ И НЕ БЫЛО (находка 6 шестого раунда ревью PR #137).
    # Прежнее условие срабатывало на ЛЮБОМ ненулевом `throttled`: один 429 на десятой секунде плюс
    # восемьдесят секунд чистых проб с `источников=0` — сервис ГЕНУИННО не вернулся, а инструмент,
    # который сам его и сломал, отчитывался «не измерено». Это ложно отрицательный вывод у
    # проверки, ЛОМАЮЩЕЙ БОЙ, — худший класс по записанному правилу направления ошибки.
    # Теперь смотрим на ПОСЛЕДНЮЮ пробу: если она дошла до движка (не 429), состояние наблюдалось,
    # и молчать про невозврат нельзя.
    if not recovered and rate_limited(after):
...
def main() -> int:
    ...
    # ⚠⚠ НАБЛЮДЕНИЕ, КОТОРОГО НЕ БЫЛО, В СЧЁТ НЕ ИДЁТ (находка 6 ревью PR #137). Сюда попадал
    # `during` ЛЮБОГО сценария — включая тот, чей вердикт «НЕ ИЗМЕРЕНО» из-за 429: `ask_plain`
    # кладёт тело отказа («Слишком много вопросов подряд…») в `text`, и оно считалось вторым
    # «сообщением сценария». Отчёт мог объявить требование базы «сообщения РАЗНЫЕ» выполненным на
    # ОДНОМ настоящем наблюдении плюс теле рейт-лимита. Ровно тот класс ложно положительных,
    # который раунд 6 закрыл в `judge` и `run_scenario`, — и он вернулся третьим местом.
    measured = {r["scenario"] for r in results if r["verdict"] != "НЕ ИЗМЕРЕНО"}
    seen = [t.strip() for sc, t in texts.items() if t.strip() and sc in measured]

...
⚠ Точный путь к файлу и готовые команды печатает драйвер: `OUT` переопределяем, и
жёсткое `/tmp/level4` увело бы оператора к несуществующему файлу (ревью раунд 3).
⚠ Каждая команда со своим stdin: `exec -T` читает его, и без `< /dev/null` съедает
остаток скрипта.
"""
...
with get_session() as db:
    ...
    # ⚠⚠ РАЗЛИЧАЮЩИЙ КОНТРОЛЬ, А НЕ «ничего не осталось» (ревью PR #135, раунд 3).
    # `left == 0` истинно И когда уборка сработала, И когда она НЕ НАШЛА НИЧЕГО — например,
    # заход шёл под аккаунтами без префикса `lt.load`. Скрипт напечатал бы успех и вышел с нулём,
    # оставив ~115 искусственных диалогов на бою: ровно то, что шаг обязан предотвращать.
    # Это тот же класс, что шаг-фикция `backup_db.py --verify` (18.08) — третий раз подряд.
    assert users, ("НЕ НАЙДЕНО НИ ОДНОГО аккаунта lt.load*. Уборка НЕ состоялась: либо заход шёл "
                   "под другими логинами, либо она уже выполнена. Проверьте вручную, прежде чем "
                   "считать бой чистым.")
    assert left == 0, f"остались нагрузочные аккаунты: {left}"
    print(f"убрано аккаунтов {len(users)}, реплик {len(msgs)}; остаток lt.load*: 0")
CLEAN

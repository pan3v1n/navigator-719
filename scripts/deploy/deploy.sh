...
ci_gate() {
  ...
  sha=$(cd "$src" && git rev-parse "$TAG^{commit}" 2>/dev/null) && from="тег $TAG"
  if [ -z "$sha" ]; then
    # Тега ещё нет — сухой прогон часто идёт ДО тегирования. Тогда HEAD честнее отсутствия
    # проверки, но об этом надо сказать вслух: проверяется не то, что поедет.
    sha=$(cd "$src" && git rev-parse HEAD 2>/dev/null) || sha=""
    from="HEAD (тега $TAG ещё нет — проверяется НЕ релизный коммит)"
  fi

...
⚠⚠ ЧЕГО ЭТА МЕТРИКА НЕ ПРОВЕРЯЕТ — ЧИТАТЬ ДО ТОГО, КАК ПОВЕРИТЬ ЕЁ ЧИСЛАМ (`P3` #121).
`search_rules` вызывается НАПРЯМУЮ, минуя `procedural.is_procedural` — то есть минуя развилку,
которая в проде решает, попадёт вопрос в этот корпус или уйдёт в товарный. Решение осознанное
(метрика обязана быть бесплатной и детерминированной), но следствие до 26.08.2026 нигде не было
записано: **«атрибуция@1 0.96» — утверждение о качестве поиска В КОРПУСЕ, а не о попадании
вопроса в корпус.**

Насколько это не теория: свипом маршрута (`scripts/eval_routing.py`) 26.08.2026 обнаружилось, что
**13 из 25 вопросов этого самого набора** живьём уходят ТОВАРНОЙ веткой — «как получить выписку из
реестра» отвечается по регистраторам данных рейса, «что делать если заявку направили на доработку»
— по подруливающим устройствам. Пока `P3` #121 открыта, числа ниже читать вместе со свипом.

...
# ⚠ Фантом, ради которого заводился гард `K15`. Настоящая страница ГИСП его не содержит; если
# он появился — значит подан ПЕРЕСКАЗ со стороннего сайта, и такой текст в корпус пускать нельзя.
_PHANTOM_RE = re.compile(r"заключени\w*\s+(?:ТПП|торгово)", re.IGNORECASE)
...
def clean(lines: list[str]) -> list[str]:
    ...
    if _PHANTOM_RE.search(flow):
        sys.exit("в тексте найдено «заключение ТПП» — это признак ПЕРЕСКАЗА со стороннего сайта, "
                 "а не страницы ГИСП. Такой документ НЕ СУЩЕСТВУЕТ (`K15`), в корпус нельзя")
    return kept

...
def iter_rows(text: str):
    ...
    Дополнительно: строка-продукт (`(из )?NN.NN|…`) ВСЕГДА начинает новую логическую
    строку — сбрасываем накопленный буфер перед ней. Иначе строки-заголовок/поправка
    без хвостового `|` (квирк некоторых разделов, напр. XXIX: дубль заголовка и «(в ред…)»
    без `|`) склеивались бы с первым продуктом, и тот, начинаясь с `#`, уходил бы в header
    и терялся (молча — verify_structured про баллы, не про состав).
    """
    buffer: list[str] = []
    for line in text.splitlines():
        if buffer and ROW_START_RE.match(line.lstrip()):
            # Перед строкой-продуктом сбрасываем буфер ТОЛЬКО если он ещё не начат кодом
            # продукта. Иначе это продолжение кодов того же продукта (коды могут идти
            # многострочно: «21.20.1,\n21.20.21|Препараты…») — его рвать нельзя.
            first = next((b for b in buffer if b.strip()), "")
            if not PRODUCT_START_RE.match(first.lstrip()):
                yield "\n".join(buffer)
                buffer = []
        buffer.append(line)
...
# ⚠ ИСКЛЮЧЁННАЯ ПОЗИЦИЯ: маркер стоит в ячейке КОДА, а не наименования, и без этой проверки
# наименованием строки становился ТЕКСТ ЯЧЕЙКИ ТРЕБОВАНИЙ. Так в корпус попали три записи
# раздела VII с именем «до 1 января 2019 г.: наличие у юридического лица…» и живыми кодами
# спецодежды (14.12.*, 20.59.52.120) — по ним рантайм отдавал целевой обрывок и ответ выходил
# БЕЗ требований вовсе, хотя реальная «Спецодежда» (14.12) лежит в разделе XVII (`D12`, #90).
# Класс тот же, что `D4`: исключённой позиции в индексе быть не должно. Прежний фильтр знал две
# формы (имя пусто; имя = заголовок раздела) и эту, третью, не ловил.
# Написаний маркера в первоисточнике ПЯТЬ (замер: 53 вхождения): «Позиция исключена» /
# «Позиции исключены» / «Код исключен», с дефисом и без, с префиксом «из», со списком кодов,
# растянутым на пять строк.
_EXCLUDED_MARK_RE = re.compile(r"(?:[Пп]озици[яи]|[Кк]од[ыа]?)\s+исключен[аыо]?")
...
def classify(fields: list[str]) -> tuple[str, object]:
    ...
    if PRODUCT_START_RE.match(f0):
        # Строка исключённой позиции: коды есть, продукции — нет. Отдаём как 'empty', иначе
        # наименованием станет текст ячейки требований (см. комментарий у _EXCLUDED_MARK_RE).
        if _EXCLUDED_MARK_RE.search(f0):
            return "empty", None
